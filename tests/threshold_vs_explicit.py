...
class TestThresholdvsExplicit:
	...
	@pytest.mark.parametrize('simulation_inputs', shared.const_inputs(linsp_n=3), ids=shared.simulation_input_idfn)
	def test_linsp(self, simulation_inputs):
		self.helper(inputs=simulation_inputs)

	@pytest.mark.extra_slow
	@pytest.mark.parametrize('simulation_inputs', shared.inputs_from_seed(10), ids=shared.simulation_input_idfn)
